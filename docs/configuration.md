...
| `RERANK_BACKEND` | `api` | `api` calls a hosted rerank endpoint (`RERANK_MODEL`); `chat` has the chat model rate each of the best `RERANK_CANDIDATES` chunks against the question, so it works with local Ollama (one model call per candidate, at most 4 in flight per process, each cut off after 20 s; a call that fails or gives no rating counts as neutral for that chunk, and if none can be rated the fused order is kept and a warning is logged). `/agent` reranks every search step and merges the steps by score, which under this backend are three coarse values that tie often: use `api` or leave reranking off with `/agent` |
